...
from lidar.shared_libs import check_for_node
...
bag_filename = "/home/elechim/Documents/D435/car_side_movement_2.bag"
...
def streamer():
    ...
    rospy.init_node('streamer', anonymous=False)
    ...
    while not rospy.is_shutdown():
        ...
        temporal =   rs2.temporal_filter(temporal_alpha.var.get(), temporal_delta.var.get(), temporal_persistency.var.get()) 
        hole_filling = rs2.hole_filling_filter(hole_filling_mode.var.get())
        ...
        if decimate_check.check():

            processed_frames = decimate.process(processed_frames).as_frameset()

        if threshold_check.check():

            processed_frames = threshold.process(processed_frames).as_frameset()

        if spatial_check.check():

            processed_frames = spatial.process(processed_frames).as_frameset()

        if temporal_check.check():

            processed_frames = temporal.process(processed_frames).as_frameset()

        if hole_filling_check.check():

            processed_frames = hole_filling.process(processed_frames).as_frameset()
