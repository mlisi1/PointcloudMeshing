...
#                                   streamer_libs                                                     #
#=====================================================================================================#
# Utility library mostly containing custom conversion functions or pointcloud processing used by
# the realsense_streamer.py node.
#------------------------------------------------------------------------------------------------------
...
#=======================#
#
# Pipeline to apply active filters to realsense frame. It relies on 
# a dict initialized in realsense_streamer.py which identifies values
# and checkboxes from the GUI
#
#---------------------------------------------------------------------
def filters_pipeline(frame, filters_dict):
    ...
    for item in filters_dict:
        ...
        if item[1].check():
            ...
            args = (val.get() for val in filters_dict[item]) if not type(filters_dict[item][0])==bool else filters_dict[item]

            processed_frames = item[0](*list(args)).process(processed_frames)
